...
| **Profile Manager** | HTTP (`/admin/profile`, bearer) | External claude-usage-monitor flips the active Claude OAuth profile (CLAUDE_CONFIG_DIR) |
...
Tables managed via `database.rs` with auto-initialization. Bootstrap with `sql/init.sql`.
